...
import endSound from '../assets/audio/end_sound.ogg'; // ✅ Adjust path as needed

const Pomodoro = ({ isPomodoroActive, setIsPromodoroActive, timeRemaining, setTimeRemaining }) => {
    ...
    const intervalCount = Number(pomodoro[0].intervalCount?.count ?? 4); // ✅ Fix for object
    ...
    const showNotification = () => {
        if (Notification.permission === 'granted') {
            new Notification('Pomodoro Timer', {
                ...
                icon: '/notification-icon.png' // ✅ use an image URL, not JSX
            });
        }
    };
    ...
    const renderIntervals = () => {
        return [...Array(intervalCount)].map((_, index) => (
            <div
                ...
                style={{
                    backgroundColor: index < completedIntervals ? 'green' : 'gray'
                }}
            >
                Interval {index + 1}
            </div>
